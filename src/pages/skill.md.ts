...
7. **Then offer to connect the solution the design needs — as a follow-up question.** Identify the Business Solution(s) the design implies — booking/calendar UI → **Bookings**; products/cart/checkout → **Stores**; contact/lead form → **Forms/CMS**; login or members area → **Members** (usually just one; only what the design actually shows — never the full set "just in case"). Then **ask the user whether to connect it now**, e.g.: *"Your site has a booking form — want me to connect it to Wix Bookings so real bookings land in your dashboard? It takes a few minutes."* **Only if they say yes**, continue with **Install the skills** below and wire that solution. If they decline, you're done — the base site already works.
   ...
   - **Stores/checkout: map the cart to real catalog refs — don't drop rows or trust client-only options.** A brought-in SPA cart keys each row by a client-only composite (e.g. \`productId + "::" + grind\`) — that key is **not** a catalog id. Check out with the **real product id** (the silent trap is a \`.filter(Boolean)\` that drops rows whose key misses the catalog map, so items vanish from the Wix order summary with no error). Sum quantities per catalog id; treat client-only options (grind/size) as cosmetic unless seeded as real variants/modifiers; and reset the checkout button's loading flag on \`pageshow\` (bfcache Back otherwise leaves it stuck on "Starting checkout…"). Verify the Wix order summary matches the on-page cart before calling it done. The installed skills carry the full recipe — \`.agents/skills/wix-headless/references/custom/ecom/WIRING.md\` (§ "Sharp edges — bridging the SPA's cart to Wix line items").
   - **Self-contained / CDN design (no module system): choose the bridge per solution.** A Claude-Design \`index.html\` runs React via CDN + Babel in \`is:inline\` scripts, so the design code **can't \`import\` the SDK** directly — but the right bridge is vertical-specific. For **Bookings** (and other elevated server writes), add small server API routes (e.g. \`src/pages/api/availability.ts\`, \`src/pages/api/book.ts\`) that use \`@wix/essentials\` \`auth.elevate(...)\` + the SDK, and point the design's mock handler at them via \`fetch()\`; verified Bookings specifics: \`createBooking\` leaves status \`CREATED\` → call \`confirmBooking\` to reach \`CONFIRMED\` (dashboard-visible), and pass explicit \`resource: { _id: service.staffMemberIds[0] }\` + \`location: { locationType: "OWNER_BUSINESS" }\` on minimally-provisioned sites. For **Stores/eCom checkout**, **do not** move checkout to a server route: the cart is visitor-owned and server-side \`checkout.createCheckout\` needs admin scope. Keep checkout client-side with visitor identity (\`currentCart.addToCurrentCart\` → \`currentCart.createCheckoutFromCurrentCart\` → \`redirects.createRedirectSession\`) from a bundled Astro \`<script>\` island (not \`is:inline\`) that exposes a \`window.<fn>\` for the CDN app to call; see \`.agents/skills/wix-headless/references/custom/ecom/WIRING.md\`.
