...
4. **Move the design in, intact.** Make it the home page **through Astro** (so the Wix runtime is injected and the SDK works client-side) — do **not** rewrite it into components or another framework:
   ...
   - **Mark every \`<script>\` from the design \`is:inline\`** (the React / Babel CDN tags and the \`type="text/babel"\` blocks) so Astro passes them through verbatim instead of bundling them. (When you regex \`<style>\`/\`<script>\` to add \`is:inline\`, run it on the page **structure only** — never over the app's JS/JSX source, or you'll corrupt tag-like tokens inside it, e.g. a JSX \`<style>{css}</style>\` becoming an invalid \`<style is:inline>\`.)
   - **If the \`index.html\` is a "bundler pack"** — its real markup/JS/fonts are base64-gzipped inside \`<script type="__bundler/manifest">\`/\`__bundler/template\` blocks, referenced by GUID (no visible \`<style>\`/\`#root\`) — **decode it first**: parse \`template\` + \`manifest\`, restore the React/ReactDOM/Babel libs as CDN \`<script>\` tags, write fonts to \`public/fonts/\` (rewrite \`url(<guid>)\`), and **concatenate the app's JS/JSX modules into one \`type="text/babel"\` block** (they share top-level scope — no \`import\`/\`export\` — so separate scripts would break; one compilation unit is required). Mime labels lie (JSX modules are often tagged \`application/javascript\`), so type every app module \`text/babel\`.
   - **Break the design into logical files — don't leave one giant \`index.astro\`.** A bundler pack already ships logical modules (data, shared components, per-page sections, app shell); even a single-file design splits along its component boundaries. Write each as \`src/design/NN-name.jsx\`, then in \`index.astro\` \`import\` each with Vite's \`?raw\` suffix and \`.join("\\n;\\n")\` into one string injected via \`<script type="text/babel" is:inline set:html={appBundle}></script>\`. Runtime is unchanged (still one Babel unit, shared scope), but the developer gets a readable, reason-about-able project instead of an 80 KB page.
